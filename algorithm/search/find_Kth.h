...
namespace myalgorithm {
    ...
    // 此种方法不能处理有重复元素的情况
    template <typename T>
    void findKth(vector<T> &v, int first, int last, int k)
    {
        // 定位中心值的索引
        int pivotIndex = partition(v, first, last, std::less<T>());
        if (pivotIndex == k) {  // 如果索引等于 k，结束查找
            return;
        } else if (k < pivotIndex) {  // 索引大于 k，则继续在左子表 [first, pivotIndex) 中查找
            findKth(v, first, pivotIndex, k);
        } else {  // 索引小于 k，则继续在右子表 [pivotIndex, last) 中查找
            findKth(v, pivotIndex + 1, last, k);
        }
    }
}
